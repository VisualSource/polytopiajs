...
import { EventDispatcher, Raycaster, WebGLRenderer, Scene, Camera, Object3D, Vector2 } from 'three';
...
class InteractionManager extends EventDispatcher {
   ...
   */
  constructor(renderer: WebGLRenderer, scene: Scene, camera: Camera, options?: {autoPreventDefault?: boolean, autoAttach?: boolean, interactionFrequency?: number}) {
    super();
   ...
   */
  hitTest(globalPoint: any, root: Object3D) {
   ...
   */
  setTargetElement(element: HTMLCanvasElement) {
    this.removeEvents();
  ...
  update({ snippet }:{snippet: number}) {
    ...
    this.setCursorMode(this.cursor as any);
   ...
   */
  setCursorMode(mode: string | null) {
    mode = mode || 'default';
   ...
   * @private
   */
  triggerEvent(displayObject: Object3D, eventString: string, eventData: any) {
   ...
   */
  processInteractive(interactionEvent: InteractionEvent, displayObject: Object3D, func: Function, hitTest: boolean, interactive?: boolean) {
...
//@ts-ignore
   ...
   */
  processClick(interactionEvent: InteractionEvent, displayObject: Object3D, hit: boolean) {
    if (hit) {
...
//@ts-ignore
   ...
   * @param {boolean} hit - the result of the hit test on the display object
   */
  processPointerDown(interactionEvent: InteractionEvent, displayObject: Object3D, hit: boolean) {
    //@ts-ignore
   ...
   */
  onPointerComplete(originalEvent: PointerEvent, cancelled: boolean, func: Function) {
    const events = this.normalizeToPointerData(originalEvent);
...
//@ts-ignore
   ...
   * @param {Object3D} displayObject - The display object that was tested
   */
  processPointerCancel(interactionEvent: InteractionEvent, displayObject: Object3D) {
    //@ts-ignore
    const data = interactionEvent.data;
//@ts-ignore
    const id = interactionEvent.data.identifier;
   ...
   * @param {boolean} hit - the result of the hit test on the display object
   */
  processPointerUp(interactionEvent: InteractionEvent, displayObject: Object3D, hit: boolean) {
    //@ts-ignore
    const data = interactionEvent.data;
//@ts-ignore
    const id = interactionEvent.data.identifier;
...
//@ts-ignore
   ...
   * @param {boolean} hit - the result of the hit test on the display object
   */
  processPointerMove(interactionEvent: InteractionEvent, displayObject: Object3D, hit: boolean) {
    //@ts-ignore
    const data = interactionEvent.data;
   ...
   */
  onPointerOut(originalEventL: PointerEvent) {
    // if we support touch events, then only use those for touch events, not pointer events
    //@ts-ignore
    if (this.supportsTouchEvents && originalEvent.pointerType === 'touch') return;
    //@ts-ignore
...
//@ts-ignore
   ...
   * @param {boolean} hit - the result of the hit test on the display object
   */
  processPointerOverOut(interactionEvent: InteractionEvent, displayObject: Object3D, hit: boolean) {
    //@ts-ignore
    const data = interactionEvent.data;
    //@ts-ignore
    const id = interactionEvent.data.identifier;
  ...
  onPointerOver(originalEvent: PointerEvent) {
    ...
    const interactionData = this.getInteractionDataForPointerId(event as any);

    const interactionEvent = this.configureInteractionEventForDOMEvent(this.eventData, event as any, interactionData);
    //@ts-ignore
    interactionEvent.data.originalEvent = event;
    //@ts-ignore
    if (event.pointerType === 'mouse') {
   ...
   */
  getInteractionDataForPointerId(event: PointerEvent) {
   ...
   */
  releaseInteractionDataForPointerId(pointerId: number) {
   ...
   */
  configureInteractionEventForDOMEvent(interactionEvent: InteractionEvent, pointerEvent: PointerEvent, interactionData: InteractionEvent) {
    //@ts-ignore
...
//@ts-ignore
   ...
   */
  normalizeToPointerData(event: TouchEvent|MouseEvent|PointerEvent) {
