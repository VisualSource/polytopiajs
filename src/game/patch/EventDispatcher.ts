 ...
 */
//@ts-ignore
EventDispatcher.prototype.on = function(type, fn: (event: Event) => void) {
 ...
 * @return {this} this
 */
EventDispatcher.prototype.off = function(type, fn: (event: Event) => void) {
 ...
 */
//@ts-ignore
EventDispatcher.prototype.once = function(type, fn: (event: Event) => void) {
  if (!Utils.isFunction(fn)) return;
  const cb = (ev: any) => {
    fn(ev);
    this.off(type, cb);
