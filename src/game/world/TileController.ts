...
export default class TileController implements SystemEventListener {
    ...
    public readonly uuid: string = nanoid();
    public events: EventEmitter = new EventEmitter();
    public position: {
        row: number;
        col: number;
    }
    public tribe_owner: string;

    constructor({engine, assets, tile_data }: ITileControllerProps){
        ...
        this.tribe_owner = tile_data.tribe;
    ...
    private selectionHandle = (event: any): void => {
        ...
        if(this.unit && this.selected === Selected.TILE){
    ...
    public async render(){
        try {
            ...
            const base_type = this.base.getType(this.tribe_owner);
            let obj = this.engine.scene.getObjectInstance(base_type);
            if(!obj) {
                const {asset,type,item} = this.base.manifest(this.tribe_owner);
            ...
            if(this.top){
                const top_type = this.top.getType(this.tribe_owner);
                let obj = this.engine.scene.getObjectInstance(top_type);
                if(!obj){
                    const {asset,item,type} = this.top.manifest(this.tribe_owner);
                    const model = await this.assets.getAsset(asset,item,type);
                    obj = this.engine.scene.createObjectInstance(base_type,model.geometry,model.material);
                }
